...
@Composable fun ProjectScreen(vm: ProjectViewModel, onBack: () -> Unit) {
    ...
    Scaffold(contentWindowInsets = WindowInsets.safeDrawing, topBar = { TopAppBar(title = { Text("项目") }, navigationIcon = { TextButton(onClick = onBack) { Text("返回") } }) }) { padding -> Column(Modifier.fillMaxSize().padding(padding).verticalScroll(rememberScrollState()).padding(24.dp)) {
        Text("保存长期背景，让下一次行动承接真实进度。", color = MaterialTheme.colorScheme.onSurfaceVariant)
